                                ...
                                {item.submenu && (
                                    <>
                                        ...
                                        <div
                                            ...
                                            ${index < 2 ? 'left-0' : index === 2 ? 'left-1/2 transform -translate-x-1/2' : 'right-0'}`}
                                        >
                                            {item.submenu.map((submenuItem, subIndex) => (
                                                <div key={subIndex} className="px-4 py-2">
                                                    <span className="block font-bold text-gray-700 dark:text-white mb-2">{submenuItem.title}</span>
                                                    {submenuItem.options.map((option, idx) => (
                                                        ...
                                                        >
                                                            ...
                                                            {option.image && <img src={option.image} alt="image" className={`${option?.width} ${option?.height} ml-2`} />}
                                                        </a>
                                                    ))}
                                                </div>
                                            ))}
                            ...
                            <div className="relative bg-white dark:bg-purple-900 p-6 w-full max-w-lg shadow-lg h-full overflow-y-auto pt-16">
                                ...
                                <nav className="space-y-4">
                                    {menu.map((item) => (
                                        <div key={item.id}>
                                            <button
                                                className={`w-full flex justify-between items-center font-bold py-2 px-4 ${openMenu === item.id ? 'bg-purple-500 text-white' : 'bg-white dark:bg-purple-900 text-black'}`}
                                            ...
                                            >
                                                ...
                                                <FaCaretDown className={`transform ${openMenu === item.id ? 'rotate-180' : 'rotate-0'}`} />
                                            </button>
